...
原有接口：
  POST /completion              → 不变，只是 AbstractMcpAgent 换了新版
"""
import json
...
# 换成新版 agent（含 HITL + revise_mcp_json）
from mcp_proxy.core.agent import AbstractMcpAgent, abstract_mcp_agent
...
async def completion(req: CompletionReq):
...
async def hitl_approve(req: HitlApproveReq):
    """
    用户同意注册 MCP Server。
...
async def hitl_reject(req: HitlRejectReq):
    """
    用户拒绝注册，分两种情况：
